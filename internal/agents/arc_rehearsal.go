...
status 表达的是“所选路径是否真的依赖它”，不是“世界里是否存在这个实体”：路径会执行该操作而世界没有对应实体/条款 → missing/unclear（这会阻断细推）；路径已决定绕开（改由公开场合、口耳相传、机制内务、本人记忆承载）→ not_required，把绕法写进 explanation，并把该缺口记入 unresolved_items。把已经绕开的操作写成 missing，等于声称计划被卡住。
...
检查关键操作的资料来源，特别是规程、许可、文书、钥匙和证据读取。“有交接夹/可查规定”不代表world_state已有可引用且具readable_facts的实体。material_checks写明operation、requires_readable、真实resource_refs及available/missing/unclear/not_required；不能造ID、补历史凭据、默认开启时间或新人物，已有资源不代表已读或条件满足。不能保证穷尽自然语言缺口，但必须检查计划中关键操作。infeasible_prediction只是预测路径不通，不是实际hard_conflict。
